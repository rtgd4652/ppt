...
- `tools/build/icon_generation/generate_artifact_icons.py` 已生成 90 个神器使专属 PNG/DDS 图标，并写入 `mod/interface/aemusa_generated_icons.gfx` 与 `mod/interface/aemusa_progression_icons.gfx`。
...
- 若 wiki 或相关公开资料中存在适合当前对象的原图，可以直接裁切、调色和缩放后放入模组；没有合适原图时，再按七日之都的都市科幻、日常与灾厄并存、青绿色 UI 光效、神器纹样、塔罗/契约/黑门意象、白夜馆和中央庭气质设计写实图。
- 图标优先保持 Stellaris 可读性：小尺寸高对比、轮廓清晰、少文字、主体居中；神器使专属图标可使用青绿、紫、金、黑的组合，但避免整套资源变成单一色调，且不要退回纯几何符号拼贴风格。
- `tools/build/icon_generation/source_atlas_realistic.png` 是当前写实图标母版；重新运行 `tools/build/icon_generation/generate_artifact_icons.py` 时会优先从母版裁切，母版缺失时才使用程序化备选图标。
- 部分 UI 会按纹理原尺寸绘制：`tr_artifact_court_*` 传统节点输出 48x48，传统分类图标和飞升图标输出 64x64，法令图标输出 32x32；科技、建筑、区划、舰船部件等图标继续使用 128x128。
- 后续缺失图标优先追加到 `tools/build/icon_generation/generate_artifact_icons.py` 的 `ICONS` 表，再重新运行生成器和引用更新脚本，避免手工散落创建。
